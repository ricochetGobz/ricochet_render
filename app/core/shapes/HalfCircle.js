...
export default class HalfCircle extends Shape {
  ...
  draw() {
    ...
    this.sprite.mask.arc( 0, 0, this.data.size, 0, Math.PI);
  ...
  animate() {
    if (!this.tweenAnimate) {
      this.tweenAnimate = TweenMax.fromTo(this, 1.5, {rotation:0}, {rotation:Math.PI * 2, yoyo:false, ease:Linear.easeNone, repeat:2, onComplete:() => {
        this.hide();
      }});
    }
